<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Co-Report</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="stylesheet" href="style.css" />
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css" />
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Libre+Baskerville:ital,wght@0,400;0,700;1,400&display=swap"
        rel="stylesheet">
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.5.1/jquery.min.js"></script>

    <style>
        @import url("https://fonts.googleapis.com/css2?family=Poppins:wght@400;600&display=swap");

        * {
            font-family: "Poppins", sans-serif;
            font-weight: 400;
            font-style: normal;

        }

        .container {
            /* width: 80%; */
            margin: 0 auto;
            padding: 80px;
            
            border: 1px solid #ccc;
            border-radius: 5px;
            background-color: rgb(166, 150, 245);
        }

        .mark {
            display: flex;
            flex-wrap: wrap;
            justify-content: space-between;
            margin-bottom: 20px;
            padding-top: 50px;
        }

        .row {
            flex: 0 0 48%;
            /* Two columns per row */
            margin-bottom: 10px;
        }

        label {
            display: block;
            margin-bottom: 5px;
        }

        select {
            width: calc(100% - 20px);
            /* Adjust select width */
            padding: 5px;
            margin-top: 5px;
            margin-bottom: 10px;
        }

        button {
            padding: 10px 20px;
            background-color: #007bff;
            color: #fff;
            border: none;
            cursor: pointer;
        }

        button:hover {
            background-color: #0056b3;
        }

        .tab {
            overflow: hidden;
            border: none;
            background-color: inherit;
            margin-bottom: 20px;
            flex: 0 0 100%;
        }

        .tab button {
            background-color: blue;
            color: white;
            display: flex;
            flex-wrap: wrap;
            flex: 1;
            justify-content: space-between;
            float: left;
            border: none;
            outline: none;
            cursor: pointer;
            padding: 14px 16px;
            transition: 0.3s;
        }

        .tab button:first-of-type {
            margin-right: 5px;
        }

        .tab button:hover {
            background-color: #45a049;
        }

        .tab button.active {
            background-color: green;
        }

        .tab .main-tab.active {
            background-color: #4CAF50;
            /* Green */
            color: white;
        }

        body {
            background-color: rgb(235, 233, 233);
            gap:50px;
        }

        /* Styles for the tab content */
        .tabcontent {
            display: none;
        }

        .tabcontent.active {
            display: block;
        }


        .dropdown-table {
            border-collapse: collapse;
            width: 100%;
        }

        .dropdown-table td,
        .dropdown-table th {
            border: 1px solid #ddd;
            padding: 8px;
        }

        .dropdown-table tr:nth-child(even) {
            background-color: #f2f2f2;
        }

        .dropdown-table tr:hover {
            background-color: #ddd;
        }

        .dropdown-table th {
            padding-top: 12px;
            padding-bottom: 12px;
            text-align: left;
            background-color: #4caf50;
            color: white;
        }

        .dropdown-table {
            width: 100%;
            border-collapse: collapse;
        }

        .dropdown-table th,
        .dropdown-table td {
            padding: 8px;
            border: 1px solid #ddd;
            text-align: left;
        }

        .dropdown-table select {
            width: 100%;
            padding: 6px 12px;
            border: 1px solid #ccc;
            border-radius: 4px;
            box-sizing: border-box;
            background-color: #fff;
            font-size: 14px;
        }

        /* Optional: Style the hover effect of the dropdown options */
        .dropdown-table select option:hover {
            background-color: #f2f2f2;
        }
        
    </style>
 
</head>
<body>
    
<div class="sidebar">
        <div class="logo"></div>
        <ul class="menu">
            <li>
                <a href="dash.php">
                    <i class="fa fa-tachometer-alt"></i>
                    <span>Dashboard</span>
                </a>
            </li>

            <li id="coDetailsNavItem" class="active">
                <a href="1stpage.php">
                    <i class="fa fa-file"></i>
                    <span>Marks</span>
                </a>
            </li>
            <li>
                <a href="co_report.php">
                    <i class="fa fa-key"></i>
                    <span>Co-Detail</span>
                </a>
            </li>
            <li>
                <a href="#">
                    <i class="fa fa-key"></i>
                    <span>Change Password</span>
                </a>
            </li>
            <li>
                <a href="testchart.php">
                    <i class="fa fa-chart-bar"></i>
                    <span>Analytics</span>
                </a>
            </li>
            <li>
                <a href="#">
                    <i class="fa fa-user"></i>
                    <span>Profile</span>
                </a>
            </li>
            <li>
                <a href="#">
                    <i class="fas fa-cog"></i>
                    <span>Settings</span>
                </a>
            </li>
            <li class="logout" id="logout">
                <a href="login.php">
                    <i class="fas fa-sign-out-alt"></i>
                    <span> Logout</span>
                </a>
            </li>
        </ul>
    </div>
    <div id="container">
    <div class="mark">

<?php
function fetchData($columnName, $tableName)
{
    $servername = "localhost";
    $username = "root";
    $password = "";
    $dbname = "cop";
    $conn = new mysqli($servername, $username, $password, $dbname);
    if ($conn->connect_error) {
        die("Connection Failed:" . $conn->connect_error);
    }

    $sql = "SELECT DISTINCT $columnName FROM $tableName";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $value = $row[$columnName];
            echo "<option value=\"$value\">$value</option>";
        }
    } else {
        echo "<option value=\"\">No $columnName found</option>";
    }

    $conn->close();
}

?>

<div class="row">
    <label for="batch">Batch</label>
    <select name="batch" id="batch">
        <option> </option>
        <?php fetchData('batch','student_info'); ?>
    </select>
</div>

<div class="row">
    <label for="course">Course</label>
    <select name="course" id="course">
        <option> </option>
        <?php fetchData('course', 'marks'); ?>
    </select>
</div>
<div class="row">
    <label for="">category</label>
    <select name="category" id="category">
    <option>Select category</option>
    <option value="subject">subject wise</option>
    <option value="batch">Batch wise</option>
    </select>
</div>
<div class="row" id="assess_type" style="display:none;">
<label for="assessment">Assessment:</label>
        <select id="assessment">
            <option></option>
            <option value="ass1">Assessment 1</option>
            <option value="ass2">Assessment 2</option>
            <option value="sem">Semester</option>
        </select>
    </div>


<div class="row" id="subjectDropdown" style="display: none;">
<label for="subject_code">Subject Code</label>
<select name="subject_code" id="subject_code">
    <option></option>
</select>
</div> 
</div>
<button onclick="submitButtonClick()">submit</button>

<script>
    
    document.getElementById('category').addEventListener('change', function () {
        var category = this.value;
        var subjectDropdown = document.getElementById("subjectDropdown");
        var subjectCodeDropdown = document.getElementById("subject_code");
        var assess_type=document.getElementById("assess_type");
        
        // Display or hide subject dropdown based on category
        if (category === "subject") {

            subjectDropdown.style.display = 'block';
            assess_type.style.display='none';
            var selectedBatch = document.getElementById("batch").value;
            var selectedCourse = document.getElementById("course").value;
            // var selectedSemester = document.getElementById("semester").value;
            
            // Clear previous options
            subjectCodeDropdown.innerHTML = "";
            
            var xhttp = new XMLHttpRequest();
            xhttp.onreadystatechange = function () {
                if (this.readyState == 4 && this.status == 200) {
                    var response = JSON.parse(this.responseText);
                    console.log(response);
                    subjectCodeDropdown.innerHTML=response.subjectCodes;
                }
                else {
                console.error("Invalid response format for subject codes");
                }       
            };
            
            // Send AJAX request to fetch subject codes and titles
            xhttp.open("GET", "subject.php?batch=" + selectedBatch + "&course=" + selectedCourse , true);
            xhttp.send();

        } else if(category === "batch") {
            subjectDropdown.style.display = 'none';
            assess_type.style.display='block';
        }
        else{
            subjectDropdown.style.display = 'none';
            assess_type.style.display='none';
        }
    });
        
        // Fetch subject codes and titles if category is subject
        // if (category === "subject") {
        //     var selectedBatch = document.getElementById("batch").value;
        //     var selectedCourse = document.getElementById("course").value;
        //     // var selectedSemester = document.getElementById("semester").value;
            
        //     // Clear previous options
        //     subjectCodeDropdown.innerHTML = "";
            
        //     var xhttp = new XMLHttpRequest();
        //     xhttp.onreadystatechange = function () {
        //         if (this.readyState == 4 && this.status == 200) {
        //             var response = JSON.parse(this.responseText);
        //             console.log(response);
        //             subjectCodeDropdown.innerHTML=response.subjectCodes;
        //         }
        //         else {
        //         console.error("Invalid response format for subject codes");
        //         }       
        //     };
            
        //     // Send AJAX request to fetch subject codes and titles
        //     xhttp.open("GET", "subject.php?batch=" + selectedBatch + "&course=" + selectedCourse , true);
        //     xhttp.send();
        // }
        
        document.getElementById('category').dispatchEvent(new Event('change'));

    function submitButtonClick() {
        var selectedcategory=document.getElementById("category").value;
        var selectedBatch = document.getElementById("batch").value;
        var selectedCourse = document.getElementById("course").value;
        if(selectedcategory === 'batch'){
        var selectedassess= document.getElementById("assessment").value;
        $.ajax({
            url: 'report_batch.php',
            type: 'POST',
            data: {
                batch: selectedBatch,
                course: selectedCourse,
                assessment:selectedassess
            },
            success: function(response) {
                var responseHtml = response;
                $('#tableContainer').html(responseHtml); // Insert response into tableContainer
                console.log("Value sent to PHP successfully");
            },
            error: function(xhr, status, error) {
                console.log("Error sending value to PHP: " + error);
            }
        });
    }
    else{
        selectedsub=document.getElementById('subject_code').value;
        $.ajax({
            url: 'report_sub.php',
            type: 'POST',
            data: {
                batch: selectedBatch,
                course: selectedCourse,
                subject_code:selectedsub
            },
            success: function(response) {
                var responseHtml = response;
                $('#tableContainer').html(responseHtml); // Insert response into tableContainer
                console.log("Value sent to PHP successfully");
            },
            error: function(xhr, status, error) {
                console.log("Error sending value to PHP: " + error);
            }
        });

    }
    }
</script>

 
    <div id="tableContainer"></div>
</body>
</html>

<!-- <script>
    document.addEventListener('DOMContentLoaded', function () {
        // Function to handle category change
        document.getElementById('category').addEventListener('change', function () {
            var category = this.value;
            var subjectDropdown = document.getElementById("subjectDropdown");
            var assessType = document.getElementById("assess_type");

            // Display or hide subject dropdown based on category
            if (category === "subject") {
                subjectDropdown.style.display = 'block';
                assessType.style.display = 'none';
            } else if (category === "batch") {
                subjectDropdown.style.display = 'none';
                assessType.style.display = 'block';
            }

            // Fetch subject codes if category is subject
            if (category === "subject") {
                var selectedBatch = document.getElementById("batch").value;
                var selectedCourse = document.getElementById("course").value;
                var subjectCodeDropdown = document.getElementById("subject_code");

                // Clear previous options
                subjectCodeDropdown.innerHTML = "";

                var xhttp = new XMLHttpRequest();
                xhttp.onreadystatechange = function () {
                    if (this.readyState == 4 && this.status == 200) {
                        var response = JSON.parse(this.responseText);
                        
                        // Add subject codes to dropdown
                        response.subjectCodes.forEach(function (code) {
                            var option = document.createElement("option");
                            option.text = code;
                            console.log(option.text)
                            option.value = code;
                            subjectCodeDropdown.appendChild(option);
                        });
                    }
                };

                // Send AJAX request to fetch subject codes
                xhttp.open("GET", "subject.php?batch=" + selectedBatch + "&course=" + selectedCourse, true);
                xhttp.send();
            }
        });

        // Initial call to handle category change on page load
        document.getElementById('category').dispatchEvent(new Event('change'));

        // AJAX request to report.php
        
    });
</script> -->



